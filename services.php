...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
	<head>
		...
		<title>MyPortal: <?php echo(!empty($SYSTEM_SETTINGS["Customise"]['UseShortOrgName']) ? 'Me and the '.$SYSTEM_SETTINGS["General"]["OrgShortName"] : $SYSTEM_SETTINGS["General"]["PortalName"]); ?></title>
				...
				<div id="page-content">
					...
					<div class="content-header">
						<div class="header-section">
							<h1><i class="<?php echo substr("fa-star", 0, 2); ?> fa-star"></i><?php echo(!empty($SYSTEM_SETTINGS["Customise"]['UseShortOrgName']) ? 'Me and the '.$SYSTEM_SETTINGS["General"]["OrgShortName"] : $SYSTEM_SETTINGS["General"]["PortalName"]); ?><br><small><?php echo $SYSTEM_SETTINGS["General"]["OrgLongName"]; ?> &middot; Self Service</small></h1>
						</div>
					</div>
					<ul class="breadcrumb breadcrumb-top">
						<?php Breadcrumbs("Home:/index.php;{$MAINMENU['items'][CONST_MENU_SERVICES]['caption']}:/services.php"); ?>
					</ul>
					<!-- END Page Header -->
...
$section = (empty($_GET['section']) ? 'subscriptions' : IdentifierStr($_GET['section']));
$titleitem = array('caption' => trim($NUCLEUS->CurrentUser->Fullname));
switch($section)
{
    case 'subscriptions':
        GuestRedirect('/index.php');
        $titleitem['caption'] = $MAINMENU['items'][CONST_MENU_SERVICES]['items'][CONST_MENUITEM_SUBSCRIPTIONS]['caption'];
        break;
/*    case 'events':
        $titleitem['caption'] = $MAINMENU['items'][CONST_MENU_EVENTS]['caption'];
        break;*/    
    default:
        $titleitem['caption'] = "{$MAINMENU['items'][CONST_MENU_SERVICES]['caption']}";        
}
BlockTitle($titleitem, 8);
switch($section)
{
    case 'subscriptions':
        Article('intro_mysubs', $NUCLEUS);
        Div(array('id' => 'MySubscriptions'), 8);
        $loadpage = "LoadContent('MySubscriptions', '/load.php?do=subscriptions', { spinner: true, urlparams: { } });";
        Div(null, 8);
        break;
/*    case 'events':
        Article(($NUCLEUS->CurrentUser->Guest ? 'intro_guestevents' : 'intro_myevents'), $NUCLEUS);
        Div(array('id' => 'MyEvents'), 8);
        $loadpage = "LoadContent('MySubscriptions', '/load.php?do=events', { spinner: true, urlparams: { } });";
        Div(null, 8);
        break;*/
    default:
}
...
$(function() {
    <?php echo $loadpage; ?>
/*    var divSubscriptions = $('#MySubscriptions');
    if ( divSubscriptions !== 'undefined' ) {
        LoadData( divSubscriptions.prop('id'), 'subscriptions', null, false);
    } 
    var divEvents = $('#MyEvents');
    if ( divEvents !== 'undefined' ) {
        LoadData( divEvents.prop('id'), 'events', null, false);
    }*/ 
});
