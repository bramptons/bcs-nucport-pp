...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
	<head>
		...
		<title>MyPortal: Membership</title>
				...
				<div id="page-content">
					...
					<div class="content-header">
						<div class="header-section">
							<h1><i class="<?php echo substr("gi-nameplate_alt", 0, 2); ?> gi-nameplate_alt"></i>Membership<br><small><?php echo $SYSTEM_SETTINGS["General"]["OrgLongName"]; ?> &middot; Self Service</small></h1>
						</div>
					</div>
					<ul class="breadcrumb breadcrumb-top">
						<?php Breadcrumbs("Home:/index.php;Membership:/membership.php"); ?>
					</ul>
					<!-- END Page Header -->
...
<?php
    ...
    NonMemberRedirect('/index.php');

//    print_r($NUCLEUS->CurrentUser);
    $titleitem = array('caption' => 'Membership');
...
<?php 
    $profile = 
    $tabitems = array();
    $tabitems['membership'] = array('id' => 'membership', 'icon' => 'gi-nameplate_alt', 'tooltip' => 'My Membership');
    $tabitems['branches'] = array('id' => 'branches', 'icon' => 'gi-git_branch', 'tooltip' => 'Branches', 'visible' => IsMenuEnabled("Membership;Branches"));
    $tabitems['directory'] = array('id' => 'directory', 'icon' => 'gi-sort', 'tooltip' => 'Directory', 'visible' => IsMenuEnabled("Membership;Directory"));
    PrepareTabs($tabitems, (!empty($_GET['activetab']) ? $_GET['activetab'] : null));
    Tabs($tabitems, 9);
?>
                                    <div class="tab-content">                                  
<?php
    if (OpenTabContent($tabitems['membership']))
    {
        EmptyFormHeading($tabitems['membership']);
        Article('intro_mymembership', $NUCLEUS);
/*        foreach($SYSTEM_SETTINGS["Customise"]["Membership"]["StatusNotices"] AS $statuscode => $settings)
        {
            if($NUCLEUS->CurrentUser->Membership->Status == $statuscode)
            {
                $alert = array(
                    'type' => $settings['type'],
                    'title' => 'Renewal due',
                    ...
                    'items' => array(
                        array(
                            'type' => 'item',
                            'caption' => ResolveVariables($settings['caption']).' ',
                        ),
                    )
                );
                if(!empty($NUCLEUS->CurrentUser->Membership->PaymentID))
                {
                    $alert['items'][] = array(
                        'type' => 'item',
                        'caption' => 'Click here to pay your renewal online or use the button below.',
                        'url' => '/finance.php?section=payment&paymentid='.$NUCLEUS->CurrentUser->Membership->PaymentID,
                    );
                }
                AlertBox($alert, 9);
                break;
        ...
        $table = array(
            ...
            'columns' => array(
                array(
                    'field' => array('name' => 'label', 'type' => 'string'),
                    'formatted' => TRUE,
                ),
                array(
        ...
        $datasource = array(
            array('label' => '<b>Your Name</b>', 'caption' => $NUCLEUS->CurrentUser->Fullname),
            array('label' => '<b>Your '.$NUCLEUS->GradeCaption.'</b>', 'caption' => $NUCLEUS->CurrentUser->Membership->MSGradeText),
            array('label' => '<b>Membership Number</b>', 'caption' => $NUCLEUS->CurrentUser->Membership->MSNumber),
            array('label' => '<b>Member since</b>', 'caption' =>date('j M Y', strtotime($NUCLEUS->CurrentUser->Membership->MSMemberSince))),
            array('label' => '<b>Next Renewal</b>', 'caption' =>date('j M Y', strtotime($NUCLEUS->CurrentUser->Membership->MSNextRenewal))),
        );
        StaticTable($datasource, $table, array('table'), 10);
        if(!empty($NUCLEUS->CurrentUser->Membership->PaymentID))
        {
            $button = array(
                'caption' => 'Pay Renewal online',
                'type' => 'button',
                'colour' => 'primary',
                'url' => '/finance.php?section=payment&paymentid='.$NUCLEUS->CurrentUser->Membership->PaymentID,
                'icon' => 'gi-gbp',
                'iconalign' => 'left',
                'tooltip' => 'Click on this button to see your payment details and make an online payment now'
            );
            Button($button, FALSE, 10);
            Para(array('margin' => TRUE), 10);
            Para(null, 10);
        }
        if(IsMenuEnabled("Membership;Directory")) {
            $dirsettings = $NUCLEUS->GetData('getdirsettings');
            $fieldsets = array();
            $fieldsets[] = array('caption' => 'Membership Directory', 'icon' => 'fa-angle-right', 'iconalign' => 'left', 'fields' => array(
            ...
            $formitem = array(
                'id' => 'frmPersonMSDirectory', 'style' => 'vertical',
                'onsubmit' => "submitForm( 'frmPersonMSDirectory', '/syscall.php?do=saveapiform&cmd=setdirsettings', { parseJSON: true, defSuccessDlg: true, defErrorDlg: true } ); return false;",
                'datasource' => $dirsettings, 'buttons' => DefFormButtons("Save Changes"),
                'fieldsets' => $fieldsets, 'borders' => TRUE
            );
            Form($formitem);
/*            RenderForm(
                $tabitems['directory']['marvin']['Form'],
                array(
                    'id' => 'frmMSSettings',
                    'maxsize' => 8,
                    'formstyle' => 'vertical',
                    'filter' => array('msdirectory'),
                    'order' => array('title', 'firstname', 'middlenames', 'lastname', 'gender', 'dob', 'nationality', 'contactemail', 'billingemail', 'altemail', 'telmobile', 'telhome', 'telwork', 'website', 'linkedin'),
                    'onsave' => "\nSendForm( { formid: 'frmMSSettings', fname: 'personal', confirm: true, invalidate: ".(empty($tabitems['directory']['marvin']['CacheID']) ? 'null' : $tabitems['directory']['marvin']['CacheID'])." } );\nreturn true;"
                )
            );*/
        }
        $listing = array(
        ...
        if (IsMenuEnabled("Membership;Branches"))
        {
            $listing['items'][] = array(
                'title' => 'Branches',
                'caption' => 'See the list of our branches and manage your own branch connections',
                'script' => "$('a[href=&quot;#tab-branches&quot;]').tab('show');",
                'urlcolour' => 'default'
            );
        }
        if (IsMenuEnabled("Services;Subscriptions"))
        {
            $listing['items'][] = array(
                'title' => 'Subscriptions',
                'caption' => 'See our list of publications and manage your subscriptions',
                'url' => "/services.php?section=subscriptions",
                'urlcolour' => 'default'
            );
        }
        if (IsMenuEnabled("Membership;Directory"))
        {
            $listing['items'][] = array(
                ...
                'caption' => 'Browse or search the Membership Directory',
                'script' => "$('a[href=&quot;#tab-directory&quot;]').tab('show');",
                'urlcolour' => 'default'
            );
        }
        if (IsMenuEnabled("CPD"))
        {
            $listing['items'][] = array(
                'title' => 'CPD',
                'caption' => 'Manage your Continuing Professional Development record',
                'url' => "/cpd.php",
                'urlcolour' => 'default'
            );
        }
        foreach($SYSTEM_SETTINGS["Customise"]["Links"] AS $linkid => $link) {
            if (CanShowLink($link) && (((!empty($link['menu']) && $link['menu'] == CONST_MENU_MEMBERSHIP)) || (empty($link['menu']) && !empty($link['members'])))) {
                $listing['items'][] = array(
                    'title' => $link['caption'],
        ...
        {
            EmptyFormHeading(array('caption' => 'Your Membership Services'));
            Div(array('class' => 'pull-down'), 9);
            Listing($listing, 10);
            Div(null, 9);
        }
        CloseTabContent($tabitems['membership']);
    }
    if (OpenTabContent($tabitems['branches']))
    {   
        EmptyFormHeading($tabitems['branches']);
        Article('intro_branches', $NUCLEUS);
        Div(array('id' => 'tblbranches'), 9);
        Div(null, 9);        
        CloseTabContent($tabitems['branches']);
    }
    if (OpenTabContent($tabitems['directory']))
    {
        EmptyFormHeading($tabitems['directory']);
        Article('intro_msdirectory', $NUCLEUS);
        $table = array('id' => 'dt_msdirectory', 'ajaxsrc' => '/datatable.php',
            'params' => array(
                'inc' => 'dtMSDirectory.php',
                'fnTotalRecords' => 'MSDirGetTotalRecords',
                'fnRow' => 'MSDirGetRow',
            ),
            'columns' => array(
                array('caption' => 'Name', 'fieldname' => 'name', 'sortable' => FALSE),
                array('caption' => $SYSTEM_SETTINGS['Customise']['Membership']['GradeCaption'], 'fieldname' => 'grade', 'sortable' => FALSE, 'hide' => array('xs', 'sm', 'md')),
                array('caption' => 'Details', 'fieldname' => 'details', 'sortable' => FALSE),
                array('caption' => 'Connect', 'searchable' => FALSE, 'sortable' => FALSE)
            ),
        );
        Datatable($table);
        CloseTabContent($tabitems['directory']);
    }
?>
                                    </div>
                                </div>
                        ...
                        <div class="col-lg-4">
<?php
portalWidgetUpcomingEvents();
portalWidgetYourProfile();
?>
                        </div>
                    </div>
                   
                    <div id="branchmodal" class="modal" tabindex="-1" role="dialog" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                                    <h3 class="modal-title">Branch Connection</h3>
                                </div>
                                <div class="modal-body">
                                    <div class="alert alert-danger alert-dismissable display-none">
                                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                                        <h4><i class="fa fa-times-circle"></i> Save Error</h4> The changes have not been saved!
                                    </div>
<?php
/*
$request = new MarvinAPIRequest('read.php', 'listbranches');
$response = $Marvin->ExecuteRequest($request);
if($response->Success)
{
    $options = array();
    $i = 0;
    while(isset($response->Outputs['branchid_'.$i]))
    {
        $options[$response->Outputs['branchid_'.$i]['value']] = $response->Outputs['branchname_'.$i]['value'];
        $i++;
    }
    $fieldsets = array(
        array('fields' => array(
                array(
                    'id' => 'branchid',
                    'name' => 'branchid',
                    'hint' => 'Select a branch to add to your branch connections',
                    'type' => 'combo',
                    'kind' => 'control',
                    'options' => $options,
                    'size' => 12, 
    ...
    $formitem = array(
        'id' => 'frmAddBranch',
        'style' => 'horizontal', 'datasource' => array(), 'buttons' => array(),
        'fieldsets' => $fieldsets, 'borders' => TRUE
    );
    Form($formitem, 9);
}
...
?>
                                ...
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-sm btn-default" data-dismiss="modal">Cancel</button>
                                    <button id="btnbranchmodal" type="button" class="btn btn-sm btn-success">Add Connection</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
$(function() {
    //LoadCollection( 'tblbranches', 'branch', 'branches', 'addbranch', 'branches', 'branchname', null );    
});
