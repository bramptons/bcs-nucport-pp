...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
	<head>
		...
		<title>MyPortal: My Applications</title>
				...
				<div id="page-content">
					...
					<div class="content-header">
						<div class="header-section">
							<h1><i class="<?php echo substr("hi-file", 0, 2); ?> hi-file"></i>My Applications<br><small><?php echo $SYSTEM_SETTINGS["General"]["OrgLongName"]; ?> &middot; Self Service</small></h1>
						</div>
					</div>
					<ul class="breadcrumb breadcrumb-top">
						<?php Breadcrumbs("Home:/index.php;My Applications:/apply.php"); ?>
					</ul>
					<!-- END Page Header -->
...
$selector = (empty($_GET['selector']) ? 'members' : IdentifierStr($_GET['selector']));
$appdata = $NUCLEUS->GetData('getapplication', array('selector' => $selector));
$NUCLEUS->StoreInSession('appdata', $appdata);
switch($selector) {
    
    default:
        UserStatusRedirect(array('member' => '/membership.php', 'guest' => '/index.php'));
}
?>
                    <div class="row">
                        <div id="appColLeft" class="col-lg-3">                         
                        </div>
                        <div id="appColMain" class="col-lg-6">
                        </div>
                        <div id="appColRight" class="col-lg-3">
                        </div>
                    </div>
...
$(function() {
    LoadAppFrames('<?php echo $selector; ?>')
/*    var inputGraduation = $('#frmStartApplication\\:Graduation');
    var inputNOY = $('#frmStartApplication\\:NOY');
    inputGraduation.on('change', function( ) {
        console.log($(this).val());
    });
    inputNOY.on('change', function() {
        console.log($(this).val());
    });*/
});
...
function LoadGrades() {
    var inputGraduation = $('#frmStartApplication\\:Graduation');
    var inputNOY = $('#frmStartApplication\\:NOY');
    var inputISO3166 = $('#frmStartApplication\\:ISO3166');
    var inputDiscountCode = $('input#frmStartApplication\\:DiscountCode');
    var inputDiscountID = $('input#frmStartApplication\\:DiscountID');
    var postparams = { Graduation: inputGraduation.val(), NOY: inputNOY.val(), ISO3166: inputISO3166.val() };
    if( inputDiscountCode.length > 0 ) {
        postparams.DiscountCode = inputDiscountCode.val();
    }
    if( inputDiscountID.length > 0 ) {
        postparams.DiscountID = inputDiscountID.val();
    }
    execSyscall(
        '/syscall.php?do=execute&cmd=eligiblegrades',
        { parseJSON: true, defErrorDlg: true, postparams: postparams,
          cbSuccess: function( response ){ 
                var selectGrades = $('#frmStartApplication\\:MSGradeID');
                selectGrades.empty();
                $.each(response.data, function(key, graderecord) {
                    //console.log(graderecord);
                    selectGrades.append($("<option></option>").attr("value", graderecord.MSGradeID).text(graderecord.GradeCaption+(graderecord.Price.length > 0 ? ", "+graderecord.Price : ""))); 
                });
                //console.log( response );
                selectGrades.trigger("chosen:updated");
          }
        }
    );
}

function LoadAppFrames(selector) {
    switch(selector) {
        case 'members':
            LoadContent('appColLeft', '/load.php?do=appframe', { spinner: true, urlparams: { column: 'left', selector: selector }, cbCompleted: function(){ LoadContent('appColLeftNavContent', '/load.php?do=appnavigation', { spinner: false, urlparams: { selector: selector } }); } });
            LoadContent('appColMain', '/load.php?do=appframe', { spinner: true, urlparams: { column: 'main', selector: selector }, cbCompleted: function(){ LoadContent('appColMainContent', '/load.php?do=apppage', { spinner: true, urlparams: { selector: selector } }); } });
            LoadContent('appColRight', '/load.php?do=appframe', { spinner: true, urlparams: { column: 'right', selector: selector } });
            break;
    }
}

function AppStarted( frmElement, jsonResponse ) {
    console.log(jsonResponse);
    switch(jsonResponse.data.selector) {
        case 'members':
            LoadContent('appColLeft', '/load.php?do=appframe', { spinner: true, urlparams: { column: 'left', selector: jsonResponse.data.selector, reload: true }, cbCompleted: function(){ LoadAppPage(jsonResponse.data.selector); } });
            LoadContent('appColRight', '/load.php?do=appframe', { spinner: true, urlparams: { column: 'right', selector: jsonResponse.data.selector } });
            //LoadContent('appColMainContent', '/load.php?do=apppage', { spinner: true, urlparams: { selector: selector } });
            break;
    }
}

function LoadAppPage(selector, page) {
    var urlparams = { selector: selector };
    if( typeof page !== 'undefined' ) {
        urlparams.page = page;
    }
    destDiv = $('#appColMainContent');
    destDiv.empty();
    destDiv.html( "<p class=\"text-center text-primary pull-down\"><i class=\"fa fa-spinner fa-5x fa-spin\"></i></p>" );
    LoadContent('appColLeftNavContent', '/load.php?do=appnavigation', { spinner: false, urlparams: urlparams, cbCompleted: function(){ LoadContent('appColMainContent', '/load.php?do=apppage', { spinner: false, urlparams: urlparams }); } } );    
}
