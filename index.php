...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
	<head>
		...
		<title>MyPortal: <?php echo $SYSTEM_SETTINGS['General']['OrgLongName']; ?></title>
				...
				<div id="page-content">
					...
					<div class="content-header content-header-media">
						<div class="header-section">
							<h1><i class="<?php echo substr("gi-home", 0, 2); ?> gi-home"></i><?php echo $SYSTEM_SETTINGS['General']['OrgLongName']; ?><br><small><?php echo $SYSTEM_SETTINGS['General']['StrapLine']; ?></small></h1>
						</div>
                        <img class="<?php echo (!empty($SYSTEM_SETTINGS['Customise']['AnimatedHeader']) ? "animation-pulseSlow" : ""); ?>" alt="header image" src="/img/<?php echo (!empty($SYSTEM_SETTINGS['Customise']['HeaderMedia']) ? $SYSTEM_SETTINGS['Customise']['HeaderMedia'] : "mainbanner.jpg"); ?>">
					</div>
...
$titleitem = array('caption' => 'Welcome'.($NUCLEUS->CurrentUser->Guest ? "" : ($isnewuser ? " ": " back, ").$NUCLEUS->CurrentUser->Firstname ));
BlockTitle($titleitem, 8);
    
//Actions list for authenticated users - common items
$list = array(
    array(
        'title' => 'Your profile',
        'caption' => 'See and edit your personal details and profile, background and expertise',
        'url' => $MAINMENU['items'][CONST_MENU_ACCOUNT]['items'][CONST_MENUITEM_PROFILE]['url'],
        'badge' => ($isnewuser ? 'Create it!' : null),
        'active' => $isnewuser
    ),
    array(
        'title' => 'My Billing',
        'caption' => 'View your transaction history, make payments and download receipts',
        'url' => $MAINMENU['items'][CONST_MENU_ACCOUNT]['items'][CONST_MENUITEM_TRANSACTIONS]['url']
    ),
);
?>
<!--Home page section for guests (not logged in)-->
<?php if($NUCLEUS->CurrentUser->Guest): ?>
                                <div class="row">
                                    <div class="col-xs-12">
                                        <div class="block-section">
<?php
Article('welcome_guest', $NUCLEUS);
?>
                                        </div>                                    
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="block-section">
                                            <h4 class="sub-header">Already got an account?</h4>
<?php
    $button = array('caption' => 'Log in now', 'type' => 'button', 'colour' => 'success', 'block' => TRUE, 'url' => "/login.php",
                    'icon' => 'fa-sign-in', 'iconalign' => 'left', 'sizeadjust' => 1);
    Button($button, FALSE, 11);
?>                                            
                                        </div>                                    
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="block-section">
                                            <h4 class="sub-header">Not registered yet?</h4>
<?php
    $button = array('caption' => 'Create an account', 'type' => 'button', 'colour' => 'primary', 'block' => TRUE, 'url' => "/login.php?register=1",
                    'icon' => 'fa-sign-in', 'iconalign' => 'left', 'sizeadjust' => 1);
    Button($button, FALSE, 11);
?>                                            
                                        </div>                                    
...
<!--Home page section for logged in members-->
<?php elseif($NUCLEUS->CurrentUser->Membership->IsMember): ?>
...
Article('welcome_member', $NUCLEUS);
foreach($SYSTEM_SETTINGS["Customise"]["Membership"]["StatusNotices"] AS $statuscode => $settings)
{
    if($NUCLEUS->CurrentUser->Membership->MSStatusID == $statuscode)
    {
        $alert = array(
            'type' => $settings['type'],
            'title' => 'Renewal due',
            ...
            'items' => array(
                array(
                    'type' => 'item',
                    'caption' => ResolveVariables($settings['caption'], $SYSTEM_SETTINGS).' ',
                ),
            )
        );
        AlertBox($alert, 9);
...
$list[] = array(
    ...
    'caption' => 'See the details of your membership and access your benefits and members-only information',
    'url' => $MAINMENU['items'][CONST_MENU_MEMBERSHIP]['url'],
    'active' => !$isnewuser,
);
Listing(array('items' => $list), 9);
?>
                                </div>
<!--Home page section for logged in non-members-->
<?php else: ?>
                                <div class="block-content">
<?php 
if($isnewuser)
...
Article('welcome_user', $NUCLEUS);
if(empty($NUCLEUS->CurrentUser->Membership->HasApplication))
{
    $list[] = array(
        'title' => 'Explore Membership',
        'caption' => 'Membership brings many benefits and access to exciting information. Click here to learn more.',
        'url' => $MAINMENU['items'][CONST_MENU_MEMBERSHIP]['url'],
    );
}
else
{
    $appdata = $NUCLEUS->GetData('getapplication', array('selector' => 'members'));
    $list[] = array(
        'title' => 'Your Application',
        'caption' => 'Thank you for your application to join the '.$SYSTEM_SETTINGS["General"]['OrgLongName'].'. '
                     .($appdata['Application']['UserCanModify'] 
                        ? 'Click here to continue your online application and submit when ready.' 
                        : ($appdata['Application']['HasTransaction'] && !$appdata['Application']['Paid']
                                ? 'Your application is awaiting payment. Click here to see its status and make an online payment.'
                                : 'Click here to follow its progress and see its current status.'
        )),
        'active' => TRUE,
        'url' => $MAINMENU['items'][CONST_MENU_MEMBERSHIP]['url'],
    );
