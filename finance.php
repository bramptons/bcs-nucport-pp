...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
	<head>
		...
		<title>MyPortal: My Billing</title>
				...
				<div id="page-content">
					...
					<div class="content-header">
						<div class="header-section">
							<h1><i class="<?php echo substr("fa-credit-card", 0, 2); ?> fa-credit-card"></i>My Billing<br><small><?php echo $SYSTEM_SETTINGS["General"]["OrgLongName"]; ?> &middot; Self Service</small></h1>
						</div>
					</div>
					<ul class="breadcrumb breadcrumb-top">
						<?php Breadcrumbs("Home:/index.php;My Billing:/finance.php"); ?>
					</ul>
					<!-- END Page Header -->
...
$hiddenAmountField = '__amount';
$hiddenDescField = '__desc';
$section = (empty($_GET['do']) ? 'mybilling' : IdentifierStr($_GET['do']));
$showcardlogos = TRUE;
switch($section){
    case 'pay':
        $loadpage = "LoadContent('finMain', '/load.php?do=pay', { spinner: true, urlparams: { InvoiceID: ".intval($_GET['InvoiceID'])." } });";
        break;
    case 'directdebit':
        $loadpage = "LoadContent('finMain', '/load.php?do=directdebit', { spinner: true, cbCompleted: function(){ LoadContent('finMainContent', '/load.php?do=ddilist', { spinner: true }); } });";
        break;
    case 'ddiwizard':
        $loadpage = "LoadContent('finMain', '/load.php?do=ddiwizard', { spinner: true });";
        $showcardlogos = FALSE;
        break;
    default:
        $loadpage = "LoadContent('finMain', '/load.php?do=mybilling', { spinner: true, cbCompleted: function(){ LoadContent('finMainContent', '/load.php?do=invoicelist', { spinner: true }); } });";
}
?>
                            </div>
<?php if($showcardlogos): ?>                            
                            <div class="block">
<?php
echo PayCardsLogos(8);
?>
                            </div>
<?php endif; ?>                            
                        </div>
...
$(function() {
    ...
    var cbDonation = $('#frmDonation\\:cbDonation');
    if( typeof cbDonation != 'undefined' ) {
        $('#frmPayment\\:_portal_donationamount').keyup(function() {
            var paymentamount = $('#frmPayment\\:_portal_amount').text().replace(/[^\d.-]/g, '');
            var donationamount = $('#frmPayment\\:_portal_donationamount').val().replace(/[^\d.]/g, '');
            var scpaymentamount = Math.floor(parseFloat(paymentamount)*100);
            if(isNaN(scpaymentamount)) { scpaymentamount = 0; 
            };
            var scdonationamount = Math.floor(parseFloat(donationamount)*100);
            if(isNaN(scdonationamount)) { scdonationamount = 0; 
            } else if(scdonationamount < 0) {
                scdonationamount = 0;
                $('#frmPayment\\:_portal_donationamount').val('');
            }
            var sctotalamount = scpaymentamount+scdonationamount;
            var totalamount = sctotalamount/100;
            $('#frmPayment\\:_portal_totalamount').html('<span class="text-primary"><b>'+$('#frmPayment\\:_portal_amount').text().substr(0, 1)+totalamount.toLocaleString()+'</b></span>');
            $('#frmPayment\\:<?php echo $hiddenAmountField; ?>').val(totalamount);
            var desc = $('#frmPayment\\:_portal_description').text();
            if (scdonationamount > 0) {
                var donation = (scdonationamount/100);
                desc = desc+'; and '+donation.toLocaleString( undefined, { currency: 'GBP', style: 'currency' })+' donation';
            }
            $('#frmPayment\\:<?php echo $hiddenDescField; ?>').text(desc);
        }).keyup();
        $('#frmDonation\\:cbDonation').change(function() {
            if ($(this).is(':checked')) {
                $('#fsdonation').show();
                $('#fstotal').show();
                $('#frmPayment\\:_portal_donationamount').keyup();
                $('#frmPayment\\:_portal_donationamount').focus();
            } else {
                var paymentamount = $('#frmPayment\\:_portal_amount').text().replace(/[^\d.-]/g, '');
                var scpaymentamount = Math.floor(parseFloat(paymentamount)*100);
                if(isNaN(scpaymentamount)) { scpaymentamount = 0; 
                ...
                $('#fsdonation').hide();
                $('#fstotal').hide();
                $('#frmPayment\\:_portal_totalamount').html($('#frmPayment\\:_portal_amount').html());
                $('#frmPayment\\:<?php echo $hiddenAmountField; ?>').val(scpaymentamount/100);
                $('#frmPayment\\:<?php echo $hiddenDescField; ?>').text( $('#frmPayment\\:_portal_description').text() );
            }
        }).change();
